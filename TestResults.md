...
flatcoin market price and Truflation's inflation measure. While a reserve backing's market price should not deviate much 
from 1.0 USD, during times of high inflation we expect it to veer away from 1.0 USD, and this will vary depending on 
how much a flatcoin's users trust it. 
...
flatcoin and its mint price will be equal to 1.0 times every flatcoin in its reserve. (All reserves start out at zero at time = 0; 
...
to a backing flatcoin will be 1.0 unit of that flatcoin.
...
every backing flatcoin. While mints and redemptions are going on, we would then suddenly raise the minting price, simulating 
...
https://github.com/PureMoney/<<<REPO_NAME_FLATCOIN>>>/blob/carlos/quick_demo/irma/programs/irma/tests/unit.rs

The redemption functionality test is much more interesting than any minting test we could think of. 

The IRMA program is designed to be fungible with respect to all backing flatcoins. In other words, once minted, each IRMA does 
not really care which flatcoin it is backed by, and it shouldn't. This is an important requirement. This implies that it should be OK 
to mint IRMA using USDT and then use the newly minted IRMA to redeem ANOTHER flatcoin, say USDC.
...
The IRMA system keeps track of the reserve backing total for each reserve flatcoin and also the amount of total IRMA in circulation 
for that flatcoin. When minting, the amount of flatcoin paid is added to the reserve backing total and the amount of IRMA minted is added 
to the total count of IRMA in circulation specific to that flatcoin. When redeeming, on the other hand, the naive way is to simply 
subtract the redeemed flatcoin amount from that flatcoin's reserve backing total, and also subtract the amount of IRMA returned 
from the total count of IRMA in circulation (specific to that flatcoin). This allows minting to adjust the redemption price towards 
...
(from the total amount in circulation) and the redeemed flatcoin amount (from the reserve backing total). The IRMA program has to protect 
...
3. The overall total IRMA in circulation is tracked, but not necessarily for each reserve flatcoin.

What item number 3 means is that a "run" can still occur for a reserve flatcoin (the IRMA system can run out of a particular flatcoin), 
but users can continue to redeem another reserve flatcoin. This protects IRMA by calculating all deviations from the mint prices and determining 
...
flatcoin with the most deviation.

The redemption test results illustrated below show how the IRMA redemption function works. 

When inflation hits above 2%, the mint prices are adjusted according to inflation and according to the price of each reserve flatcoin. Here
...
prices for the other flatcoins increase. This should be an effective dis-incentive.
...
On the other hand, IRMA in circulation for each flatcoin reserve changes for every redemption. Because redemption price is simply
reserve total divided by IRMA in circulation for a flatcoin, notice how the redemption price in the second graph above changes
to approach the mint price for each flatcoin, even when no minting is going on.
