...
async function initializeProtocol() {
  ...
  try {
    ...
    try {
      const existingState = await (program.account as any).stateMap.fetch(statePda);
      console.log("ℹ️ Protocol already initialized!");
      console.log("📊 Existing state:", existingState);
      return { state: existingState, core: null };
    } catch (error) {
      console.log("📝 State not found, proceeding with initialization...");
    }
