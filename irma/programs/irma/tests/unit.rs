...
mod tests {
    ...
    fn test_set_mint_price_anchor() {
        ...
        let result = irma::irma::set_mint_price(
            Context::new(program_id, &mut accounts, &[], MaintBumps::default()),
            "USDT".to_string(), 1.5);
        assert!(result.is_ok());
    ...
    fn test_redeem_irma_anchor() -> Result<()> {
        ...
        let reserves = accounts_mut.state.reserves.clone();
        for sc in reserves {
            ...
            let mut_backing = accounts_mut.state.get_mut_stablecoin(&sc.symbol).unwrap();
        ...
        msg!("Mid-state for USDT before further redemption: {:?}", 
            irma::irma::list_reserves(Context::new(program_id, &mut accounts_mut, &[], MaintBumps::default())));

        let mut result = redeem_irma(&mut accounts_mut.state, "USDC", 10);
        match &result {
            Err(e) => {
        ...
        result = redeem_irma(&mut accounts_mut.state, "USDT", 20);
        match &result {
            Err(e) => {
        ...
        assert!(result.is_ok(), "Redeem IRMA failed for USDT");

        result = redeem_irma(&mut accounts_mut.state, "PYUSD", 30);
        match &result {
            Err(e) => {
        ...
        assert!(result.is_ok(), "Redeem IRMA failed for PYUSD");

        result = redeem_irma(&mut accounts_mut.state, "USDG", 40);
        match &result {
            Err(e) => {
        ...
        assert!(result.is_ok(), "Redeem IRMA failed for USDG");

        result = redeem_irma(&mut accounts_mut.state, "FDUSD", 50);
        match &result {
            Err(e) => {
        ...
        assert!(result.is_ok(), "Redeem IRMA failed for FDUSD");

        // sale trade event
        result = irma::irma::sale_trade_event(
            Context::new(program_id, &mut accounts_mut, &[], MaintBumps::default()),
            "USDT".to_string(), 50_000_000
        );
        assert!(result.is_ok());

        // buy trade event
        result = irma::irma::buy_trade_event(
            Context::new(program_id, &mut accounts_mut, &[], MaintBumps::default()),
            "USDT".to_string(), 10_000
        );
        match &result {
            Err(e) => {
                msg!("Error redeeming IRMA for USDT: {:?}", e);
        ...
        result = redeem_irma(&mut accounts_mut.state, "USDS", 10);
        match &result {
            Err(e) => {
        ...
        msg!("Backing reserves: {:?}", accounts_mut.state.reserves);
        Ok(())
    }
